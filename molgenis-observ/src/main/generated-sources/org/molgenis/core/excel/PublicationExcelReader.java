...
/**
 * Reads Publication from Excel file.
 */
public class PublicationExcelReader
{
	public static final transient Logger logger = Logger.getLogger(PublicationExcelReader.class);
			
	/**
	 * Imports Publication from a workbook sheet.
	 */
	...
	{
		File tmpPublication = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpPublication.txt");
		if(tmpPublication.exists()){
			boolean deleteSuccess = tmpPublication.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpPublication.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpPublication.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpPublication.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpPublication);
		if(fileHasHeaders){
			int count = new PublicationCsvReader().importCsv(db, tmpPublication, defaults, dbAction, missingValue);
			tmpPublication.delete();
			return count;
		}else{
			tmpPublication.delete();
			return 0;
