...
/**
 * Reads MolgenisFile from Excel file.
 */
public class MolgenisFileExcelReader
{
	public static final transient Logger logger = Logger.getLogger(MolgenisFileExcelReader.class);
			
	/**
	 * Imports MolgenisFile from a workbook sheet.
	 */
	...
	{
		File tmpMolgenisFile = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpMolgenisFile.txt");
		if(tmpMolgenisFile.exists()){
			boolean deleteSuccess = tmpMolgenisFile.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpMolgenisFile.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpMolgenisFile.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpMolgenisFile.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpMolgenisFile);
		if(fileHasHeaders){
			int count = new MolgenisFileCsvReader().importCsv(db, tmpMolgenisFile, defaults, dbAction, missingValue);
			tmpMolgenisFile.delete();
			return count;
		}else{
			tmpMolgenisFile.delete();
			return 0;
