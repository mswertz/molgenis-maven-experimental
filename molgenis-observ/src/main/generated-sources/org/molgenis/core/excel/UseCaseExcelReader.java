...
/**
 * Reads UseCase from Excel file.
 */
public class UseCaseExcelReader
{
	public static final transient Logger logger = Logger.getLogger(UseCaseExcelReader.class);
			
	/**
	 * Imports UseCase from a workbook sheet.
	 */
	...
	{
		File tmpUseCase = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpUseCase.txt");
		if(tmpUseCase.exists()){
			boolean deleteSuccess = tmpUseCase.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpUseCase.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpUseCase.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpUseCase.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpUseCase);
		if(fileHasHeaders){
			int count = new UseCaseCsvReader().importCsv(db, tmpUseCase, defaults, dbAction, missingValue);
			tmpUseCase.delete();
			return count;
		}else{
			tmpUseCase.delete();
