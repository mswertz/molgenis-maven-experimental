...
/**
 * Reads RuntimeProperty from Excel file.
 */
public class RuntimePropertyExcelReader
{
	public static final transient Logger logger = Logger.getLogger(RuntimePropertyExcelReader.class);
			
	/**
	 * Imports RuntimeProperty from a workbook sheet.
	 */
	...
	{
		File tmpRuntimeProperty = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpRuntimeProperty.txt");
		if(tmpRuntimeProperty.exists()){
			boolean deleteSuccess = tmpRuntimeProperty.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpRuntimeProperty.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpRuntimeProperty.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpRuntimeProperty.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpRuntimeProperty);
		if(fileHasHeaders){
			int count = new RuntimePropertyCsvReader().importCsv(db, tmpRuntimeProperty, defaults, dbAction, missingValue);
			tmpRuntimeProperty.delete();
			return count;
		}else{
			tmpRuntimeProperty.delete();
			return 0;
