...
/**
 * Reads MolgenisRole from Excel file.
 */
public class MolgenisRoleExcelReader
{
	public static final transient Logger logger = Logger.getLogger(MolgenisRoleExcelReader.class);
			
	/**
	 * Imports MolgenisRole from a workbook sheet.
	 */
	...
	{
		File tmpMolgenisRole = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpMolgenisRole.txt");
		if(tmpMolgenisRole.exists()){
			boolean deleteSuccess = tmpMolgenisRole.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpMolgenisRole.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpMolgenisRole.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpMolgenisRole.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpMolgenisRole);
		if(fileHasHeaders){
			int count = new MolgenisRoleCsvReader().importCsv(db, tmpMolgenisRole, defaults, dbAction, missingValue);
			tmpMolgenisRole.delete();
			return count;
		}else{
			tmpMolgenisRole.delete();
			return 0;
