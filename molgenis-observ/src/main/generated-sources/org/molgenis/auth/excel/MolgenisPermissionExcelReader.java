...
/**
 * Reads MolgenisPermission from Excel file.
 */
public class MolgenisPermissionExcelReader
{
	public static final transient Logger logger = Logger.getLogger(MolgenisPermissionExcelReader.class);
			
	/**
	 * Imports MolgenisPermission from a workbook sheet.
	 */
	...
	{
		File tmpMolgenisPermission = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpMolgenisPermission.txt");
		if(tmpMolgenisPermission.exists()){
			boolean deleteSuccess = tmpMolgenisPermission.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpMolgenisPermission.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpMolgenisPermission.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpMolgenisPermission.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpMolgenisPermission);
		if(fileHasHeaders){
			int count = new MolgenisPermissionCsvReader().importCsv(db, tmpMolgenisPermission, defaults, dbAction, missingValue);
			tmpMolgenisPermission.delete();
			return count;
		}else{
			tmpMolgenisPermission.delete();
			return 0;
