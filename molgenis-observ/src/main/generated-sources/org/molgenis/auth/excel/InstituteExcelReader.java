...
/* File:        observ/model/Institute.java
...
/**
 * Reads Institute from Excel file.
 */
public class InstituteExcelReader
{
	public static final transient Logger logger = Logger.getLogger(InstituteExcelReader.class);
			
	/**
	 * Imports Institute from a workbook sheet.
	 */
	...
	{
		File tmpInstitute = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpInstitute.txt");
		if(tmpInstitute.exists()){
			boolean deleteSuccess = tmpInstitute.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpInstitute.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpInstitute.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpInstitute.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpInstitute);
		if(fileHasHeaders){
			int count = new InstituteCsvReader().importCsv(db, tmpInstitute, defaults, dbAction, missingValue);
			tmpInstitute.delete();
			return count;
		}else{
			tmpInstitute.delete();
