...
/* File:        Observ/html/Category.java
...
public class CategoryForm extends EntityForm<Category>
{
	...
	{	
		...
		//Description: Field(entity=Category, name=description, type=text, auto=false, nillable=false, readonly=false, default=)
		{
			TextInput input = new TextInput("Category_description",getEntity().getDescription());
			
			input.setLabel("description");
			input.setDescription("Description of the code.     Use of ontology terms references to establish unambigious descriptions is     recommended");
			input.setNillable(false);
			input.setReadonly( isReadonly() || getEntity().isReadonly());
			if(this.getHiddenColumns().contains(input.getName()))
			{	
		...
		//Investigation: Field(entity=ObservationElement, name=Investigation, type=xref[Investigation.id], xref_label='name', auto=false, nillable=true, readonly=false, default=)
		{
			...
			//XrefInput input = new XrefInput("Category_Investigation", getEntity().getInvestigation());
			...
			XrefInput input = new XrefInput("Category_Investigation", org.molgenis.organization.Investigation.class, dummy);
		...
		{
			//TODO: when we have JPA this should become:
			//MrefInput input = new MrefInput("Category_ontologyReference", getEntity().getOntologyReference());
			...
			MrefInput input = new MrefInput("Category_ontologyReference", org.molgenis.core.OntologyTerm.class, dummyList);
		...
		{
			//TODO: when we have JPA this should become:
			//MrefInput input = new MrefInput("Category_AlternateId", getEntity().getAlternateId());
			//create xref dummy list of references
			...
			MrefInput input = new MrefInput("Category_AlternateId", org.molgenis.pheno.AlternateId.class, dummyList);
		...
		{
			StringInput input = new StringInput("Category_label",getEntity().getLabel());
		...
		//Code_String: Field(entity=Category, name=code_string, type=string[255], auto=false, nillable=false, readonly=false, default=)
		{
			StringInput input = new StringInput("Category_code_string",getEntity().getCode_String());
			
			input.setLabel("code_string");
			input.setDescription("The code used to represent this category. For example: { &apos;1&apos; codes for &apos;male&apos;, &apos;2&apos;-&apos;female&apos;}");
			input.setNillable(false);
			input.setSize(255);
			input.setReadonly( isReadonly() || getEntity().isReadonly());
			if(this.getHiddenColumns().contains(input.getName()))
		...
		//IsMissing: Field(entity=Category, name=isMissing, type=bool, auto=false, nillable=false, readonly=false, default=false)
		{
			...
			input.setDescription("whether this code should be treated as missing value.");
			input.setNillable(false);
			input.setReadonly( isReadonly() || getEntity().isReadonly());
			if(this.getHiddenColumns().contains(input.getName()))
			{	
