...
/* File:        Observ/html/ObservationTarget.java
...
/**
 * A HtmlForm that is preloaded with all inputs for entity ObservationTarget
 * @see EntityForm
 */
public class ObservationTargetForm extends EntityForm<ObservationTarget>
{
	...
	public ObservationTargetForm(ObservationTarget entity)
	{
		super(entity);
	}
	...
	@Override
	public Class<ObservationTarget> getEntityClass()
	{
		return ObservationTarget.class;
	}
	...
	{	
		...
		//Id: Field(entity=ObservationTarget, name=id, type=int, auto=true, nillable=false, readonly=true, default=)
		{
			IntInput input = new IntInput("ObservationTarget_id",getEntity().getId());
		...
		{
			StringInput input = new StringInput("ObservationTarget_name",getEntity().getName());
		...
		{
			TextInput input = new TextInput("ObservationTarget_description",getEntity().getDescription());
		...
		//Investigation: Field(entity=ObservationElement, name=Investigation, type=xref[Investigation.id], xref_label='name', auto=false, nillable=true, readonly=false, default=)
		{
			...
			//XrefInput input = new XrefInput("ObservationTarget_Investigation", getEntity().getInvestigation());
			...
			XrefInput input = new XrefInput("ObservationTarget_Investigation", org.molgenis.organization.Investigation.class, dummy);
		...
		{
			//TODO: when we have JPA this should become:
			//MrefInput input = new MrefInput("ObservationTarget_ontologyReference", getEntity().getOntologyReference());
			...
			MrefInput input = new MrefInput("ObservationTarget_ontologyReference", org.molgenis.core.OntologyTerm.class, dummyList);
		...
		{
			EnumInput input = new EnumInput("ObservationTarget___Type",getEntity().get__Type());
		...
		{
			//TODO: when we have JPA this should become:
			//MrefInput input = new MrefInput("ObservationTarget_AlternateId", getEntity().getAlternateId());
			//create xref dummy list of references
			...
			MrefInput input = new MrefInput("ObservationTarget_AlternateId", org.molgenis.pheno.AlternateId.class, dummyList);
		...
		{
			StringInput input = new StringInput("ObservationTarget_label",getEntity().getLabel());
