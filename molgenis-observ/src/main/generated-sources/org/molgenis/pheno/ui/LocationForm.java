...
/* File:        Observ/html/Location.java
...
/**
 * A HtmlForm that is preloaded with all inputs for entity Location
 * @see EntityForm
 */
public class LocationForm extends EntityForm<Location>
{
	...
	public LocationForm(Location entity)
	{
		super(entity);
	}
	...
	{	
		...
		//Id: Field(entity=Location, name=id, type=int, auto=true, nillable=false, readonly=true, default=)
		{
			IntInput input = new IntInput("Location_id",getEntity().getId());
		...
		{
			StringInput input = new StringInput("Location_name",getEntity().getName());
		...
		{
			TextInput input = new TextInput("Location_description",getEntity().getDescription());
		...
		//Investigation: Field(entity=ObservationElement, name=Investigation, type=xref[Investigation.id], xref_label='name', auto=false, nillable=true, readonly=false, default=)
		{
			...
			//XrefInput input = new XrefInput("Location_Investigation", getEntity().getInvestigation());
			...
			XrefInput input = new XrefInput("Location_Investigation", org.molgenis.organization.Investigation.class, dummy);
		...
		{
			//TODO: when we have JPA this should become:
			//MrefInput input = new MrefInput("Location_ontologyReference", getEntity().getOntologyReference());
			...
			MrefInput input = new MrefInput("Location_ontologyReference", org.molgenis.core.OntologyTerm.class, dummyList);
		...
		{
			EnumInput input = new EnumInput("Location___Type",getEntity().get__Type());
		...
		{
			//TODO: when we have JPA this should become:
			//MrefInput input = new MrefInput("Location_AlternateId", getEntity().getAlternateId());
			//create xref dummy list of references
			...
			MrefInput input = new MrefInput("Location_AlternateId", org.molgenis.pheno.AlternateId.class, dummyList);
		...
		{
			StringInput input = new StringInput("Location_label",getEntity().getLabel());
