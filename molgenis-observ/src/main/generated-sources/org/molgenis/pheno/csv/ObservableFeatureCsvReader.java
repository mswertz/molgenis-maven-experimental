...
{
	public static final transient Logger logger = Logger.getLogger(ObservableFeatureCsvReader.class);
	...
	public int importCsv(final Database db, CsvReader reader, final Tuple defaults, final DatabaseAction dbAction, final String missingValues) throws DatabaseException, IOException, Exception 
	{
		...
		final List<ObservableFeature> observableFeaturesMissingRefs = new ArrayList<ObservableFeature>();
		...
		for(Tuple tuple: reader)
		{
			...
			if(observableFeatureList.size() == BATCH_SIZE)
			{
				...
				db.update(observableFeatureList,dbAction, "name");
		...
		List<ObservableFeature> observableFeaturesStillMissingRefs = resolveForeignKeys(db, observableFeaturesMissingRefs);
		
		//if there are still missing references, throw error and rollback
		if(observableFeaturesStillMissingRefs.size() > 0){
			throw new Exception("Import of 'ObservableFeature' objects failed: attempting to resolve in-list references, but there are still ObservableFeatures referring to ObservableFeatures that are neither in the database nor in the list of to-be imported ObservableFeatures. (the first one being: "+observableFeaturesStillMissingRefs.get(0)+")");
		}
		//else update the entities in the database with the found references and return total
		else
		{				
			db.update(observableFeaturesMissingRefs,DatabaseAction.UPDATE, "name");
	...
	private List<ObservableFeature> resolveForeignKeys(Database db, List<ObservableFeature> observableFeatureList) throws Exception
	{
		...
		for(ObservableFeature o: observableFeatureList) for(String xref_label: o.getOntologyReference_Name())
		{
			if(xref_label != null) 
		...
		for(ObservableFeature o: observableFeatureList) for(String xref_label: o.getAlternateId_Name())
		{
			if(xref_label != null) 
		...
		for(ObservableFeature o:  observableFeatureList)
		{
			while(true){
				...
				if(o.getInvestigation_Name() != null) 
				{
					...
					{
						throw new Exception("Import of 'ObservableFeature' objects failed: cannot find Investigation for investigation_name='"+o.getInvestigation_Name()+"'");
					}
				...
				if(o.getOntologyReference_Name() != null) 
				{
					...
					for(int i = 0; i < listSize; i++)
					{
						...
						if(ontologyReferenceKeymap.get(key) == null){
							...
							throw new Exception("Import of 'ObservableFeature' objects failed: cannot find ontologyReference_name='"+(o.getOntologyReference_Name() != null && i < o.getOntologyReference_Name().size() ? o.getOntologyReference_Name().get(i) : "null")+"'");
						}
				...
				if(o.getAlternateId_Name() != null) 
				{
					...
					for(int i = 0; i < listSize; i++)
					{
						...
						if(alternateIdKeymap.get(key) == null){
							logger.error("Import of 'ObservableFeature' objects failed: "+o);
							throw new Exception("Import of 'ObservableFeature' objects failed: cannot find alternateId_name='"+(o.getAlternateId_Name() != null && i < o.getAlternateId_Name().size() ? o.getAlternateId_Name().get(i) : "null")+"'");
						}
