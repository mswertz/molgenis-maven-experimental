...
/**
 * Reads Individual from Excel file.
 */
public class IndividualExcelReader
{
	public static final transient Logger logger = Logger.getLogger(IndividualExcelReader.class);
			
	/**
	 * Imports Individual from a workbook sheet.
	 */
	...
	{
		File tmpIndividual = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpIndividual.txt");
		if(tmpIndividual.exists()){
			boolean deleteSuccess = tmpIndividual.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpIndividual.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpIndividual.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpIndividual.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpIndividual);
		if(fileHasHeaders){
			int count = new IndividualCsvReader().importCsv(db, tmpIndividual, defaults, dbAction, missingValue);
			tmpIndividual.delete();
			return count;
		}else{
			tmpIndividual.delete();
			return 0;
