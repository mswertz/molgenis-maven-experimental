...
/**
 * Reads Measurement from Excel file.
 */
public class MeasurementExcelReader
{
	public static final transient Logger logger = Logger.getLogger(MeasurementExcelReader.class);
			
	/**
	 * Imports Measurement from a workbook sheet.
	 */
	...
	{
		File tmpMeasurement = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpMeasurement.txt");
		if(tmpMeasurement.exists()){
			boolean deleteSuccess = tmpMeasurement.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpMeasurement.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpMeasurement.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpMeasurement.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpMeasurement);
		if(fileHasHeaders){
			int count = new MeasurementCsvReader().importCsv(db, tmpMeasurement, defaults, dbAction, missingValue);
			tmpMeasurement.delete();
			return count;
		}else{
			tmpMeasurement.delete();
			return 0;
