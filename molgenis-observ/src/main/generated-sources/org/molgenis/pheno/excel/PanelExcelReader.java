...
/**
 * Reads Panel from Excel file.
 */
public class PanelExcelReader
{
	public static final transient Logger logger = Logger.getLogger(PanelExcelReader.class);
			
	/**
	 * Imports Panel from a workbook sheet.
	 */
	...
	{
		File tmpPanel = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpPanel.txt");
		if(tmpPanel.exists()){
			boolean deleteSuccess = tmpPanel.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpPanel.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpPanel.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpPanel.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpPanel);
		if(fileHasHeaders){
			int count = new PanelCsvReader().importCsv(db, tmpPanel, defaults, dbAction, missingValue);
			tmpPanel.delete();
			return count;
		}else{
			tmpPanel.delete();
