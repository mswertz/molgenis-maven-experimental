...
/**
 * Reads Category from Excel file.
 */
public class CategoryExcelReader
{
	public static final transient Logger logger = Logger.getLogger(CategoryExcelReader.class);
			
	/**
	 * Imports Category from a workbook sheet.
	 */
	...
	{
		File tmpCategory = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpCategory.txt");
		if(tmpCategory.exists()){
			boolean deleteSuccess = tmpCategory.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpCategory.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpCategory.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpCategory.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpCategory);
		if(fileHasHeaders){
			int count = new CategoryCsvReader().importCsv(db, tmpCategory, defaults, dbAction, missingValue);
			tmpCategory.delete();
			return count;
		}else{
			tmpCategory.delete();
