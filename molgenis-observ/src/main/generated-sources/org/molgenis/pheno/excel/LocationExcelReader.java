...
/**
 * Reads Location from Excel file.
 */
public class LocationExcelReader
{
	public static final transient Logger logger = Logger.getLogger(LocationExcelReader.class);
			
	/**
	 * Imports Location from a workbook sheet.
	 */
	...
	{
		File tmpLocation = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpLocation.txt");
		if(tmpLocation.exists()){
			boolean deleteSuccess = tmpLocation.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpLocation.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpLocation.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpLocation.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpLocation);
		if(fileHasHeaders){
			int count = new LocationCsvReader().importCsv(db, tmpLocation, defaults, dbAction, missingValue);
			tmpLocation.delete();
			return count;
		}else{
			tmpLocation.delete();
