...
/**
 * Reads ObservationElement from Excel file.
 */
public class ObservationElementExcelReader
{
	public static final transient Logger logger = Logger.getLogger(ObservationElementExcelReader.class);
			
	/**
	 * Imports ObservationElement from a workbook sheet.
	 */
	...
	{
		File tmpObservationElement = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpObservationElement.txt");
		if(tmpObservationElement.exists()){
			boolean deleteSuccess = tmpObservationElement.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpObservationElement.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpObservationElement.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpObservationElement.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpObservationElement);
		if(fileHasHeaders){
			int count = new ObservationElementCsvReader().importCsv(db, tmpObservationElement, defaults, dbAction, missingValue);
			tmpObservationElement.delete();
			return count;
		}else{
			tmpObservationElement.delete();
			return 0;
