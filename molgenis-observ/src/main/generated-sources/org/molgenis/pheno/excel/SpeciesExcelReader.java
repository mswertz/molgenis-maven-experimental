...
/**
 * Reads Species from Excel file.
 */
public class SpeciesExcelReader
{
	public static final transient Logger logger = Logger.getLogger(SpeciesExcelReader.class);
			
	/**
	 * Imports Species from a workbook sheet.
	 */
	...
	{
		File tmpSpecies = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpSpecies.txt");
		if(tmpSpecies.exists()){
			boolean deleteSuccess = tmpSpecies.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpSpecies.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpSpecies.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpSpecies.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpSpecies);
		if(fileHasHeaders){
			int count = new SpeciesCsvReader().importCsv(db, tmpSpecies, defaults, dbAction, missingValue);
			tmpSpecies.delete();
			return count;
		}else{
			tmpSpecies.delete();
