...
/**
 * Reads ObservationTarget from Excel file.
 */
public class ObservationTargetExcelReader
{
	public static final transient Logger logger = Logger.getLogger(ObservationTargetExcelReader.class);
			
	/**
	 * Imports ObservationTarget from a workbook sheet.
	 */
	...
	{
		File tmpObservationTarget = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpObservationTarget.txt");
		if(tmpObservationTarget.exists()){
			boolean deleteSuccess = tmpObservationTarget.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpObservationTarget.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpObservationTarget.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpObservationTarget.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpObservationTarget);
		if(fileHasHeaders){
			int count = new ObservationTargetCsvReader().importCsv(db, tmpObservationTarget, defaults, dbAction, missingValue);
			tmpObservationTarget.delete();
			return count;
		}else{
			tmpObservationTarget.delete();
			return 0;
