...
/**
 * Reads Protocol_Features from Excel file.
 */
public class Protocol_FeaturesExcelReader
{
	public static final transient Logger logger = Logger.getLogger(Protocol_FeaturesExcelReader.class);
			
	/**
	 * Imports Protocol_Features from a workbook sheet.
	 */
	...
	{
		File tmpProtocol_Features = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpProtocol_Features.txt");
		if(tmpProtocol_Features.exists()){
			boolean deleteSuccess = tmpProtocol_Features.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpProtocol_Features.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpProtocol_Features.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpProtocol_Features.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpProtocol_Features);
		if(fileHasHeaders){
			int count = new Protocol_FeaturesCsvReader().importCsv(db, tmpProtocol_Features, defaults, dbAction, missingValue);
			tmpProtocol_Features.delete();
			return count;
		}else{
			tmpProtocol_Features.delete();
			return 0;
