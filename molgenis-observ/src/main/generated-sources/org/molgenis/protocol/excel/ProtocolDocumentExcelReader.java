...
/**
 * Reads ProtocolDocument from Excel file.
 */
public class ProtocolDocumentExcelReader
{
	public static final transient Logger logger = Logger.getLogger(ProtocolDocumentExcelReader.class);
			
	/**
	 * Imports ProtocolDocument from a workbook sheet.
	 */
	...
	{
		File tmpProtocolDocument = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpProtocolDocument.txt");
		if(tmpProtocolDocument.exists()){
			boolean deleteSuccess = tmpProtocolDocument.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpProtocolDocument.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpProtocolDocument.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpProtocolDocument.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpProtocolDocument);
		if(fileHasHeaders){
			int count = new ProtocolDocumentCsvReader().importCsv(db, tmpProtocolDocument, defaults, dbAction, missingValue);
			tmpProtocolDocument.delete();
			return count;
		}else{
			tmpProtocolDocument.delete();
			return 0;
