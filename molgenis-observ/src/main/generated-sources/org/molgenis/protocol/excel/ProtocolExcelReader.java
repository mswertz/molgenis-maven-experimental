...
/**
 * Reads Protocol from Excel file.
 */
public class ProtocolExcelReader
{
	public static final transient Logger logger = Logger.getLogger(ProtocolExcelReader.class);
			
	/**
	 * Imports Protocol from a workbook sheet.
	 */
	...
	{
		File tmpProtocol = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpProtocol.txt");
		if(tmpProtocol.exists()){
			boolean deleteSuccess = tmpProtocol.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpProtocol.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpProtocol.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpProtocol.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpProtocol);
		if(fileHasHeaders){
			int count = new ProtocolCsvReader().importCsv(db, tmpProtocol, defaults, dbAction, missingValue);
			tmpProtocol.delete();
			return count;
		}else{
			tmpProtocol.delete();
