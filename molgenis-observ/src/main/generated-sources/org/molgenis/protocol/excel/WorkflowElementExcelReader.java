...
/**
 * Reads WorkflowElement from Excel file.
 */
public class WorkflowElementExcelReader
{
	public static final transient Logger logger = Logger.getLogger(WorkflowElementExcelReader.class);
			
	/**
	 * Imports WorkflowElement from a workbook sheet.
	 */
	...
	{
		File tmpWorkflowElement = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpWorkflowElement.txt");
		if(tmpWorkflowElement.exists()){
			boolean deleteSuccess = tmpWorkflowElement.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpWorkflowElement.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpWorkflowElement.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpWorkflowElement.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpWorkflowElement);
		if(fileHasHeaders){
			int count = new WorkflowElementCsvReader().importCsv(db, tmpWorkflowElement, defaults, dbAction, missingValue);
			tmpWorkflowElement.delete();
			return count;
		}else{
			tmpWorkflowElement.delete();
			return 0;
