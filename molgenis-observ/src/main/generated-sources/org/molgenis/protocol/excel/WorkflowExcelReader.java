...
/**
 * Reads Workflow from Excel file.
 */
public class WorkflowExcelReader
{
	public static final transient Logger logger = Logger.getLogger(WorkflowExcelReader.class);
			
	/**
	 * Imports Workflow from a workbook sheet.
	 */
	...
	{
		File tmpWorkflow = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpWorkflow.txt");
		if(tmpWorkflow.exists()){
			boolean deleteSuccess = tmpWorkflow.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpWorkflow.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpWorkflow.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpWorkflow.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpWorkflow);
		if(fileHasHeaders){
			int count = new WorkflowCsvReader().importCsv(db, tmpWorkflow, defaults, dbAction, missingValue);
			tmpWorkflow.delete();
			return count;
		}else{
			tmpWorkflow.delete();
