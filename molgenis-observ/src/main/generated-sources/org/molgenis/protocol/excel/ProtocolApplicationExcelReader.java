...
/**
 * Reads ProtocolApplication from Excel file.
 */
public class ProtocolApplicationExcelReader
{
	public static final transient Logger logger = Logger.getLogger(ProtocolApplicationExcelReader.class);
			
	/**
	 * Imports ProtocolApplication from a workbook sheet.
	 */
	...
	{
		File tmpProtocolApplication = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpProtocolApplication.txt");
		if(tmpProtocolApplication.exists()){
			boolean deleteSuccess = tmpProtocolApplication.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpProtocolApplication.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpProtocolApplication.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpProtocolApplication.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpProtocolApplication);
		if(fileHasHeaders){
			int count = new ProtocolApplicationCsvReader().importCsv(db, tmpProtocolApplication, defaults, dbAction, missingValue);
			tmpProtocolApplication.delete();
			return count;
		}else{
			tmpProtocolApplication.delete();
			return 0;
