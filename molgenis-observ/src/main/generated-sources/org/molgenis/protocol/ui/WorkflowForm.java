...
/* File:        Observ/html/Workflow.java
...
/**
 * A HtmlForm that is preloaded with all inputs for entity Workflow
 * @see EntityForm
 */
public class WorkflowForm extends EntityForm<Workflow>
{
	...
	public WorkflowForm(Workflow entity)
	{
		super(entity);
	}
	...
	{	
		...
		//Id: Field(entity=Workflow, name=id, type=int, auto=true, nillable=false, readonly=true, default=)
		{
			IntInput input = new IntInput("Workflow_id",getEntity().getId());
		...
		//Name: Field(entity=Workflow, name=name, type=string[127], auto=false, nillable=false, readonly=false, default=)
		{
			StringInput input = new StringInput("Workflow_name",getEntity().getName());
		...
		{
			RichtextInput input = new RichtextInput("Workflow_description",getEntity().getDescription());
		...
		//Investigation: Field(entity=Protocol, name=Investigation, type=xref[Investigation.id], xref_label='name', auto=false, nillable=true, readonly=false, default=)
		{
			...
			//XrefInput input = new XrefInput("Workflow_Investigation", getEntity().getInvestigation());
			...
			XrefInput input = new XrefInput("Workflow_Investigation", org.molgenis.organization.Investigation.class, dummy);
		...
		{
			//TODO: when we have JPA this should become:
			//MrefInput input = new MrefInput("Workflow_ontologyReference", getEntity().getOntologyReference());
			...
			MrefInput input = new MrefInput("Workflow_ontologyReference", org.molgenis.core.OntologyTerm.class, dummyList);
		...
		{
			EnumInput input = new EnumInput("Workflow___Type",getEntity().get__Type());
		...
		{
		    //TODO: when we have JPA this should become:
			//XrefInput input = new XrefInput("Workflow_ProtocolType", getEntity().getProtocolType());
			//create xref dummy object
			...
			XrefInput input = new XrefInput("Workflow_ProtocolType", org.molgenis.core.OntologyTerm.class, dummy);
		...
		{
			//TODO: when we have JPA this should become:
			//MrefInput input = new MrefInput("Workflow_Features", getEntity().getFeatures());
			//create xref dummy list of references
			...
			MrefInput input = new MrefInput("Workflow_Features", org.molgenis.pheno.ObservableFeature.class, dummyList);
		...
		{
			StringInput input = new StringInput("Workflow_TargetFilter",getEntity().getTargetFilter());
		...
		{
		    //TODO: when we have JPA this should become:
			//XrefInput input = new XrefInput("Workflow_Contact", getEntity().getContact());
			//create xref dummy object
			Person dummy = null;
			...
			XrefInput input = new XrefInput("Workflow_Contact", org.molgenis.auth.Person.class, dummy);
		...
		{
			//TODO: when we have JPA this should become:
			//MrefInput input = new MrefInput("Workflow_subprotocols", getEntity().getSubprotocols());
			//create xref dummy list of references
			...
			MrefInput input = new MrefInput("Workflow_subprotocols", org.molgenis.protocol.Protocol.class, dummyList);
