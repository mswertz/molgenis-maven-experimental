...
/*${name(entity)}<#if entity.hasExtends()> extends ${name(entity.getExtends())}</#if><#if entity.hasImplements()> implements <#list entity.getImplements() as i>${name(i)}<#if i_has_next>,</#if></#list></#if>*/
...
<#list dbFields(entity) as f>
	...
	<#if f.auto && f.type == "int" && ( !entity.getExtends()?exists || !entity.getExtends().getField(f.name)?exists )> AUTO_INCREMENT</#if></@compress>
</#list>
...
ALTER TABLE ${SqlName(entity)} <@compress single_line=true>
	...
	<#if !f.nillable> NOT </#if>NULL
	<#if f.getDefaultValue()?exists && f.getDefaultValue() != "" && f.type != "text" && f.type != "blob" && f.type != "hyperlink"> DEFAULT <#if f.type == "bool" || f.type == "int">${f.getDefaultValue()}<#else>"${f.getDefaultValue()}"</#if></#if>
	<#if f.auto && f.type == "int" && ( !entity.getExtends()?exists || !entity.getExtends().getField(f.name)?exists )> AUTO_INCREMENT</#if>
</@compress> IF EXISTS;
...
<#-- Generate a table for each concrete class (so, not abstract) -->
<#list entities as entity><#if !entity.isAbstract() && entity.hasExtends()>
ALTER TABLE ${SqlName(entity)} ADD FOREIGN KEY (${SqlName(pkey(entity))}) REFERENCES ${SqlName(entity.getExtends())} (${SqlName(pkey(entity))}) ON DELETE RESTRICT;
</#if></#list>
