 ...
 */
<#if entity.abstract>
public interface ${JavaName(entity)} extends <#if entity.hasImplements()><#list entity.getImplements() as i> ${i.namespace}.${JavaName(i)}<#if i_has_next>,</#if></#list><#else>org.molgenis.db.Entity</#if>
<#else>
...
<#foreach index in entity.indices>
	...
	"${field}"<#if field_has_next>,</#if>
		...
		<#if !entity.hasExtends() && entity.hasDescendants() >
...
public class ${JavaName(entity)} extends <#if entity.hasExtends()>${entity.getExtends().namespace}.${JavaName(entity.getExtends())}<#else>org.molgenis.db.common.AbstractEntity</#if> <#if entity.hasImplements()>implements<#list entity.getImplements() as i> ${i.namespace}.${JavaName(i)}<#if i_has_next>,</#if></#list></#if>
</#if>
{
<#if entity.abstract>
<#--interface only has method signatures-->
	<#foreach field in entity.getImplementedFields()>
		...
		<#if (field.name != typefield()) || !entity.hasExtends()>
	...
	public void set${JavaName(field)}(<#if field.type = "xref">${field.xrefEntity.namespace}.${JavaName(field.xrefEntity)}<#else>${type(field)}</#if> ${name(field)});
		<#if type_label == "enum">
	public java.util.List<org.molgenis.db.common.ValueLabel> get${JavaName(field)}Options();
	...
	public void set${JavaName(field)}_${JavaName(field.xrefLabel)}(String ${name(field)}_${name(field.xrefLabel)});
	...
	public void set${JavaName(field)}_${JavaName(f.xrefField)}(java.util.List<${type(f.xrefField)}> ${JavaName(field)}_${JavaName(f.xrefField)}List);	
			<#if field.xrefLabel.name != field.xrefFieldName>
	public java.util.List<String> get${JavaName(field)}_${JavaName(field.xrefLabel)}();
	public void set${JavaName(field)}_${JavaName(field.xrefLabel)}(java.util.List<String> ${name(field)}_${field.xrefLabel}List);	
			</#if>						
...
<#else>
	...
	// fieldname constants
    <#foreach field in entity.getImplementedFields()>
	public final static String ${field.name?upper_case} = "${field.name}";<#if field.type == "xref" || field.type == "mref">
	public final static String ${field.name?upper_case}_${field.xrefLabel.name?upper_case} = "${field.name}_${field.xrefLabel.name}";</#if>
	</#foreach>
	 ...
	 */
	public static org.molgenis.db.Query<? extends ${JavaName(entity)}> query(org.molgenis.db.Database db)
	{
		return db.query(${JavaName(entity)}.class);
	}
	
	/**
	 * Shorthand for db.find(${JavaName(entity)}.class, org.molgenis.db.QueryRule ... rules).
	 */
	public static java.util.List<? extends ${JavaName(entity)}> find(org.molgenis.db.Database db, org.molgenis.db.QueryRule ... rules) throws org.molgenis.db.common.DatabaseException
	{
		return db.find(${JavaName(entity)}.class, rules);
	}	
	
<#foreach key in entity.allUniques>	
	/**
	 * ${key.entity.name}
	 */
	public static ${JavaName(entity)} findBy<#list key.fields as f>${JavaName(f)}</#list>(org.molgenis.db.Database db<#list key.fields as f>, ${type(f)} ${name(f)}</#list>) throws org.molgenis.db.common.DatabaseException
	{
		org.molgenis.db.Query<${JavaName(entity)}> q = db.query(${JavaName(entity)}.class);
	...
	<#if !isPrimaryKey(field,entity) || !entity.hasExtends()>
 			<#if isPrimaryKey(field,entity) && !entity.hasExtends()>
    			<#if field.auto = true>
	    			<#if jpa_use_sequence >
...
//	@org.hibernate.search.annotations.Field(index=org.hibernate.search.annotations.Index.TOKENIZED, store=org.hibernate.search.annotations.Store.NO)
    ...
    @javax.persistence.JoinColumn(name="${SqlName(field)}", insertable=true, updatable=true, nullable=${field.nillable?string})
			<#if multipleXrefs &gt; 1>
...
//	@javax.persistence.Lob()
	...
	@javax.persistence.Transient
	private java.util.List<org.molgenis.db.common.ValueLabel> ${name(field)}_options = new java.util.ArrayList<org.molgenis.db.common.ValueLabel>();
		<#elseif type_label == "xref">
	@javax.persistence.Transient
	private ${type(field.xrefField)} ${name(field)}_${name(field.xrefField)} = ${default(field)};	
	
	@javax.persistence.Transient
	private ${type(field.xrefLabel)} ${name(field)}_${name(field.xrefLabel)} = null;						
	...
	private java.util.List<${type(field.xrefField)}> ${name(field)}_${name(field.xrefField)} = new java.util.ArrayList<${type(field.xrefField)}>();		
			<#if field.xrefLabel.name != field.xrefFieldName>
	@javax.persistence.Transient
	private java.util.List<String> ${name(field)}_${field.xrefLabel.name} = new java.util.ArrayList<String>();
			</#if>	
			<#elseif type_label == "file" || type_label=="image" >
	@javax.persistence.Lob
	...
	<#list entity.getFields() as f>
		...
		${name(f)}_options.add(new org.molgenis.db.common.ValueLabel("${option}","${option}"));
			</#list>
		</#if>	
	</#list>
	}
	...
	@Deprecated
	public <#if field.type =="xref">${field.xrefEntity.namespace}.${JavaName(field.xrefEntity)}<#elseif field.type == "mref">java.util.List<${field.xrefEntity.namespace}.${JavaName(field.xrefEntity)}><#else>${type(field)}</#if> get${JavaName(field)}(org.molgenis.db.Database db)
	{
	...
	public void set${JavaName(field)}( <#if field.type =="xref">${field.xrefEntity.namespace}.${JavaName(field.xrefEntity)}<#elseif field.type == "mref">java.util.List<${field.xrefEntity.namespace}.${JavaName(field.xrefEntity)}><#else>${type(field)}</#if> ${name(field)})
	{
		...
		<#if entity.hasExtends()> 
			<#--if entity.getExtends().getField(field.getName(), false, true, true)?exists>
				//hack to solve problem with variable hidden in supertype
				super.set${JavaName(field)}(${name(field)});
			</#if-->
			<#if entity.getExtends().getAllField(field.getName())?exists>
...
<#if field.xrefLabel.name != field.xrefFieldName>
	/**
	 * Get a pretty label ${field.xrefLabel.name} for cross reference ${JavaName(field)} to ${JavaName(field.xrefEntity)}.${JavaName(field.xrefField)}.
	 */
	public ${type(field.xrefLabel)} get${JavaName(field)}_${JavaName(field.xrefLabel)}()
	{		
		//FIXME should we auto-load based on get${JavaName(field)}()?	
		if(${name(field)} != null) {
			return ${name(field)}.get${JavaName(field.xrefLabel)}();
		} else {
			return ${name(field)}_${name(field.xrefLabel)};
		}
	}		
	 ...
	 */
	public void set${JavaName(field)}_${JavaName(field.xrefLabel)}(${type(field.xrefLabel)} ${name(field)}_${name(field.xrefLabel)})
	{
		this.${name(field)}_${name(field.xrefLabel)} = ${name(field)}_${name(field.xrefLabel)};
	}		
</#if>
...
<#if field.xrefLabel.name != field.xrefFieldName>	
	 ...
	 */
	public java.util.List<${type(field.xrefLabel)}> get${JavaName(field)}_${JavaName(field.xrefLabel)}()
	{
<#if databaseImp == "JPA">
		if(this.${name(field)} != null && this.${name(field)}.size() > 0)
		{
			java.util.List<${type(field.xrefLabel)}> result = new java.util.ArrayList<${type(field.xrefLabel)}>();
			for(${field.xrefEntity.namespace}.${JavaName(field.xrefEntity)} o: ${name(field)}) result.add(o.get${JavaName(field.xrefLabel)}().toString());
...
<#else>
	 ...
	 */
	public void set${JavaName(field)}_${JavaName(field.xrefLabel)}(java.util.List<String> ${name(field)}_${name(field.xrefLabel)})
	{
		this.${name(field)}_${name(field.xrefLabel)} = ${name(field)}_${name(field.xrefLabel)};
	}		
</#if>		
...
<#if field.xrefLabel.name != field.xrefFieldName>
		if(name.toLowerCase().equals("${name(field)?lower_case}_${name(field.xrefLabel)?lower_case}"))
			return get${JavaName(field)}_${JavaName(field.xrefLabel)}();
</#if>			
		</#if>
	</#foreach>		
		return "";
	...
	<#list allFields(entity) as field><#if field.nillable == false>
		if(this.get${JavaName(field)}() == null) throw new org.molgenis.db.common.DatabaseException("required field ${name(field)} is null");
	</#if></#list>
	}
...
<#list allFields(entity) as field>
	<#assign type_label = field.getType().toString()>
		...
		result+= " ${name(field)}_${name(field.xrefField)}='" + get${JavaName(field)}_${JavaName(field.xrefField)}()+"' ";	
			<#if field.xrefLabel.name != field.xrefFieldName>		
		result+= " ${name(field)}_${name(field.xrefLabel)}='" + get${JavaName(field)}_${JavaName(field.xrefLabel)}()+"' ";
			</#if>
		<#else>
	...
	<#list allFields(entity) as field>
		...
		<#if field.type="xref" || field.type="mref">
			<#if field.xrefLabel.name != field.xrefFieldName>
		fields.add("${name(field)}_${name(field.xrefLabel)}");
			</#if>
		</#if>
	</#list>		
		return fields;
	...
	public java.util.List<String> getLabelFields()
	{
		...
		<#if entity.getXrefLabel()?exists>
		result.add("${entity.getXrefLabel()}");
		</#if>
		return result;
	}
...
<#list entity.getUniqueKeysWithoutPk() as uniqueKeys >
	...
	@Override
	public ${JavaName(entity)} create(org.molgenis.db.Tuple tuple) throws Exception
	{
...
<#list model.entities as e>
	<#if !e.abstract && !e.association>
		<#list e.implementedFields as f>
			<#if f.type=="xref" && f.xrefEntity.name == entity.name>
...
<#list model.entities as e>
	<#if !e.abstract && !e.association>
		<#list e.implementedFields as f>
			<#if f.type=="mref" && f.xrefEntity.name == entity.name>
				<#assign multipleXrefs = model.getNumberOfReferencesTo(entity)/>
	//${multipleXrefs}
	...
	@javax.xml.bind.annotation.XmlTransient
	public java.util.Collection<${f.entity.namespace}.${Name(f.entity)}> get${Name(f)}<#if multipleXrefs &gt; 1 >${Name(f.entity)}</#if>Collection(org.molgenis.db.Database db)
	{
