...
/* File:        auth/model/MolgenisUser.java
...
/**
 * Reads MolgenisUser from Excel file.
 */
public class MolgenisUserExcelReader
{
	public static final transient Logger logger = Logger.getLogger(MolgenisUserExcelReader.class);
			
	/**
	 * Imports MolgenisUser from a workbook sheet.
	 */
	...
	{
		File tmpMolgenisUser = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpMolgenisUser.txt");
		if(tmpMolgenisUser.exists()){
			boolean deleteSuccess = tmpMolgenisUser.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpMolgenisUser.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpMolgenisUser.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpMolgenisUser.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpMolgenisUser);
		if(fileHasHeaders){
			int count = new MolgenisUserCsvReader().importCsv(db, tmpMolgenisUser, defaults, dbAction, missingValue);
			tmpMolgenisUser.delete();
			return count;
		}else{
			tmpMolgenisUser.delete();
			return 0;
