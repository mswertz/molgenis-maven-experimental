...
/**
 * Reads MolgenisGroup from Excel file.
 */
public class MolgenisGroupExcelReader
{
	public static final transient Logger logger = Logger.getLogger(MolgenisGroupExcelReader.class);
			
	/**
	 * Imports MolgenisGroup from a workbook sheet.
	 */
	...
	{
		File tmpMolgenisGroup = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpMolgenisGroup.txt");
		if(tmpMolgenisGroup.exists()){
			boolean deleteSuccess = tmpMolgenisGroup.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpMolgenisGroup.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpMolgenisGroup.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpMolgenisGroup.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpMolgenisGroup);
		if(fileHasHeaders){
			int count = new MolgenisGroupCsvReader().importCsv(db, tmpMolgenisGroup, defaults, dbAction, missingValue);
			tmpMolgenisGroup.delete();
			return count;
		}else{
			tmpMolgenisGroup.delete();
			return 0;
